...
//   npm run forge:onboard -- "Bright Smile Dental" "A gentle family dental practice in Jersey City"
async function main() {
  const name = process.argv[2];
  const description = process.argv.slice(3).join(' ');
  if (!name || !description) {
    console.error('Usage: npm run forge:onboard -- "<business name>" "<one-paragraph description>"');
    console.error(
      'Example: npm run forge:onboard -- "Bright Smile Dental" "A gentle family dental practice focused on anxiety-free care"',
    );
    process.exit(1);
  }
  ...
  console.log(`\nNext: npm run forge:run -- ${client.slug} "Write 3 launch posts"`);
}

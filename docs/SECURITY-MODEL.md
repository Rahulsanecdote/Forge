...
RLS is configured deny-all — `anon` and `authenticated` are revoked on every table, with
grants only to `service_role`, and a trigger enables RLS on new tables — but that
configuration is a *backstop against a leaked anon key*, not the mechanism that keeps
clients apart.
...
The one intentional exception to deny-all: `leads` grants `INSERT` to `anon`, so the
marketing contact form can write without a server round-trip. It grants no `SELECT`.
...
- **Twilio** (`/api/twilio/inbound`) — HMAC-SHA1 over the request URL plus sorted POST
  ...
  opt-outs, so forging it would let someone suppress a competitor's review requests, or
  un-suppress somebody who opted out.
...
variables are exposed to the client, and the anon key is safe to publish precisely because
RLS denies it everything.
...
- **Idempotent publishing** — a durable checkpoint is claimed before each external call, so
  a crash mid-publish cannot silently double-post. An ambiguous outcome goes to an operator
  reconciliation queue instead of being retried blindly.

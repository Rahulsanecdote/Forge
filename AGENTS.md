...
| 2026-07-23 | Per-client portal revocation: portal login keys and session cookies are now signed with a per-client secret derived from the global secret + client id + `clients.portal_key_version` (`portalClientSecret`). `session.ts` reads the client's current version (parsing the client id from the cookie first, unverified, only to load it) and verifies against the version-derived secret, so an operator "Revoke & rotate" that bumps one client's version invalidates just that client's links/sessions and rotates their link — other clients untouched. Rotating `FORGE_PORTAL_SECRET` remains the global kill switch. Column-missing pre-migration degrades to version 1. |
| 2026-07-22 | Hygiene: `data/keywords.ts` reads DataForSEO config lazily from `process.env` instead of importing `@/env`, so its pure unit tests (and build-time page collection) no longer hard-exit without SUPABASE_* — the full `npm run test` suite is green again. The Stripe webhook resolves `current_period_end` from the subscription or, on newer API versions (2025-03+), the latest subscription item. |
